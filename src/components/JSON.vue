...
<template>
  <div class="json-lab">
    ...
    <section class="lab-section">
      ...
      <p v-for="author in orwell" :key="author.id">Finding by property: {{ author?.name }}</p>

      <h3>Nested Arrays/Objects</h3>
      <p v-for="author in austen" :key="author.id">{{ author?.name }}'s works:</p>
      <!-- Activity 9: Render a list of Austen's works. Hint: Use the v-for directive to iterate through the array of authors that you have filtered out. -->
      <!-- TODO: CODE TO RENDER LIST OF AUSTEN'S WORKS HERE -->
      <ul v-for="author in austen" :key="author.id">
    ...
    <section class="lab-section">
      ...
      <p>
        ...
        <span>{{ bookstoresData.totalStores }}</span>
      </p>
      ...
      <ul>
        <li v-for="(count, type) in bookstoresData.storeTypes" :key="type">
          {{ type }}: {{ count }} stores
        </li>
      </ul>
      ...
      <ul>
        <li v-for="(times, days) in bookstoresData.openingHours" :key="days">
          {{ days }}:
      ...
      <p>We operate in: 
        {{ bookstoresData.countries.join(', ') }}
      </p>
      <p>Our #1 seller:
        <!-- <ul>
          <li v-for="topSeller in bookstoresData.topSellers" :key="topSeller">
            {{ topSeller }}
          </li>
        </ul> -->
        <span>{{ bookstoresData.topSellers[0] }}</span>
      </p>
    </section>
...
// const change = () => fontWeight.value = fontWeight.value === "bold" ? "normal": "bold"

// Activity 2: Get authors born after 1850
const modernAuthors = computed(() => authors.filter(author => author.birthYear > 1850))

// Activity 3: Get all famous works
const allFamousWorks = computed(() => authors.flatMap(author => author.famousWorks.map(famousWork => famousWork.title)))
...
const bookstoresData = bookstores
